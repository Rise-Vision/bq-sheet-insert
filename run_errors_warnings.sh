...
# 4) Comment out the first setting of CONFIG_FILES and uncomment second setting
# 5) In command line run the following: ./run_errors_warnings.sh [path/to/service/acct.json]

# CONFIG_FILES=(financial-errors-warnings-config.js rss-errors-warnings-config.js weather-errors-warnings-config.js)
# CONFIG_FILES=(image-errors-warnings-config.js slides-errors-warnings-config.js video-errors-warnings-config.js riseplayerconfiguration-errors-warnings-config.js)
